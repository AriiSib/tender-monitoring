...
public class TelegramNotificationService {
    ...
    public void notifyUser(List<TrackedTender> trackedTenders, Long chatId) {
        for (TrackedTender trackedTender : trackedTenders) {
            String message = buildMessage(trackedTender, chatId);
            telegramBot.send(chatId, message);
        }
    }

    private String buildMessage(TrackedTender tender, Long chatId) {
        StringBuilder message = new StringBuilder();
        if (tender != null && chatId != null) {
            message.append("🔔 Найден новый тендер:\n\n");
            ...
            if (tender.getExpirationDate() != null) {
                message.append("\n🗓 Окончание: ").append(fmt.format(tender.getExpirationDate())).append("\n");
            } else {
                message.append("\n");
            }
